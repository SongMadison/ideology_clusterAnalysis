...
shinyServer(function(input, output) {
    ...
    createCorr <- reactive({
        ...
        balloon.plot(cov, xlabel = colnames(data1), 
                     ...
                     ,main = paste0("correlation coefficients among variables-",input$`data set`))
    })    
    ...
    createCenterPlot1 <- reactive({
        ...
        plot1 + theme(panel.grid.major=element_line(colour="black",size = 0.1))
    })
    ...
    createCenterPlot12 <- reactive({
        ...
        plot2 <-  plt.centers(x1, p_clus)
        #grid.arrange(plot1, plot2, ncol = 1)
        plot2 + theme(panel.grid.major=element_line(colour="black",size = 0.1))
    })
    ...
    output$centers_plot12 <- renderPlot({ 
        print (createCenterPlot12())   
    })
