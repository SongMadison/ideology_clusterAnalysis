...
#library(foreign) # read.spss outdated
#library(memisc)  #use this one 

# cleaning data
# read  .sav, removing missing  -- > data1 , standardize the data -- > x1

#originalData = foreign::read.spss("./MCRC1617_CLUSTER VARS.sav", to.data.frame = TRUE)
...
x1 <- scale(data1); 
# x1 <- apply(data1, MARGIN = 2, function(x) x/max(x))
